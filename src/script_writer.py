...
def _write_verification(
    ...
    for c in checks:
        ev = "\n".join(
            f"    - {'追い風' if e['relation'] == 'support' else '逆風'}: {e['claim']}"
            for e in c.get("evidence_today", [])
        ) or "    - （今日は、この論点を動かすような材料は出ていません）"
        said = "\n".join(f"    - {s}" for s in c.get("already_said", [])) \
            or "    - （まだありません）"
        rows.append(f"""
## {c['id']}（{c.get('axis', '')}）
論点: {c['statement'].strip()}
これまでの見え方: {c.get('stance', '')}
今日の動き: {c.get('shift', '')}
今日の材料:
{ev}
この見方が変わるとしたら（何が観測されたときか）:
{chr(10).join('    - ' + t for t in c.get('falsifier_texts', []))}
{('この観測に当たる出来事がありました: ' + ' / '.join(c['falsifier_hits'])) if c.get('falsifier_hits') else ''}
この論点が効いてくる判断: {c.get('decision_link', '')}
過去の放送ですでに話した根拠（**二度と使わないでください**）:
{said}
""")

    prompt = f"""{date_label}の音声ニュース番組の「大きな流れ」コーナーの台本を書いてください。

この番組は、生産・流通・販売にまたがる大きな論点をいくつか持っていて、
その日のニュースがその論点のどこに当たるのかを、毎朝ひとつだけ確かめています。
細かい数値の当てっこをするコーナーではありません。
**その日のニュースを大きな絵のどこに置くか**を示すのが目的です。

今日扱うのは次の{len(checks)}件です。
...
# このコーナーの書き方（厳守）
- **確からしさの数字は絶対に読み上げないでください。**
  「0.55から0.65になりました」のような言い方は禁止です。パーセントも使いません。
  代わりに「支持する材料のほうが多い」「まだどちらとも言えない」のように言葉で言います。
- 話の順番はこの形にしてください。
  1) 論点を1文で言い直す（前回の言い回しをなぞらず、今日の切り口で言い直す）
  2) 今日のニュースがこの論点の追い風か、逆風か、どちらでもないかを言う
  3) そう言い切れない部分、別の説明のつきかたを必ず1つ挙げる
  4) この見方が変わるとしたら何が観測されたときかを、1つだけ挙げる
- **「過去の放送ですでに話した根拠」は使わないでください。** 同じ事例・同じ数字を
  繰り返すのが、このコーナーのいちばんの失敗です。今日の材料だけで話してください。
- 今日の材料が空の場合は、無理に材料をひねり出さないでください。
  「今日はこの論点を動かす材料は出ませんでした」と正直に言い、そのうえで
  「材料が出ないこと自体をどう読むか」を短く話してください。
- 特定の一次産品（例：ある国の漁獲量）の細部に踏み込みすぎないでください。
  それは個別ニュースのコーナーの仕事です。ここでは構造の話に留めます。
- {cfg['hosts']['analyst']['label']}が見立てを述べ、{cfg['hosts']['navigator']['label']}が疑う側に立ちます。
